...
def remove_right_units(string):
    ...
    # this does not seem to be True ? I am seeing a multiple choice question with \text{(A)} in the answer choice in the test set
    if "\\text{ " in string:
...
def strip_string(string):
    ...
    # To see whether we need to add this or not
    # # remove \text{...} because I am seeing \text{(D)} in test set -> (D). Only do that when at least one letter is present
    # string = re.sub(r"\\text\{([^}]*[A-Za-z][^}]*)\}", r"\1", string)
    # # remove \mathrm{...} wrapper
    # string = re.sub(r"\\mathrm\{(.*?)\}", r"\1", string)
    # # strip surrounding parentheses for single-letter answers, e.g. (D) -> D
