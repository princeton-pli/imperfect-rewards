...
from open_instruct.IFEval import instructions_registry
...
class VerificationResult:
    ...
    extra_logging_scores: dict[str, float] | None = None
...
class VerifierFunction(ABC):
    ...
    async def async_call(
        ...
        Returns:
            VerificationResult
        """
        logger.debug(f"[{self.name}.async_call] Starting verification")
        try:
            loop = asyncio.get_running_loop()
            result = await loop.run_in_executor(None, lambda: self.__call__(tokenized_prediction, prediction, label, query))
            logger.debug(f"[{self.name}.async_call] Completed with score={result.score}")
            return result
        except Exception as e:
            logger.error(f"[{self.name}.async_call] Error: {e}")
            raise
...
class IFEvalVerifier(VerifierFunction):
    ...
    Returns per-constraint scores in constraint_scores dict for weighted aggregation.
    """
    ...
    def __call__(
        ...
        if len(prediction) == 0 or len(answer) == 0:
            ...
            for instruction_key in instruction_keys:
                constraint_scores[instruction_key] = 0.0
            return VerificationResult(score=0.0, constraint_scores=constraint_scores)
        for instruction_key, args in zip(instruction_keys, args_list):
            ...
            if prediction.strip() and instruction_instance.check_following(answer):
                rewards.append(1.0)
                constraint_scores[instruction_key] = 1.0
            else:
                rewards.append(0.0)
                constraint_scores[instruction_key] = 0.0
        return VerificationResult(score=sum(rewards) / len(rewards), constraint_scores=constraint_scores)
...
class CodeVerifier(VerifierFunction):
    ...
    def __init__(self, verifier_config: CodeVerifierConfig) -> None:
        ...
        self.binary_training = verifier_config.binary_training
    ...
    async def async_call(
        ...
        try:
            ...
            if self.binary_training:
                ...
                extra_scores = {"pass_rate": pass_rate}
            else:
                score = 0.0 if pass_rate < self.pass_rate_reward_threshold else pass_rate
                extra_scores = {"binary_pass_rate": 0.0 if pass_rate < 1.0 else 1.0}
            ...
            return VerificationResult(score=score, extra_scores=extra_scores)
        except Exception as e:
...
async def apply_verifiable_reward(
    ...
    """
    Apply verifiable rewards to responses.
    
    Args:
        reward_fn_mapping: Dict mapping dataset names to verifier functions
        responses: Tokenized responses
        decoded_responses: Decoded string responses
        ground_truths: Ground truth labels
        datasets: Dataset names for each response
        reward_mult: Multiplier for rewards (verification_reward)
        queries: Original queries (optional)
        verifier_weights: Per-verifier/per-constraint weights (optional)
    """
    if queries is None:
    ...
    logger.debug(f"[apply_verifiable_reward] inside reward_fn with {len(responses)} responses")
    if len(responses) > 1:
        logger.debug(f"[apply_verifiable_reward] example of response: {decoded_responses[0][:200]}...")

    is_same_length = len(responses) == len(decoded_responses) == len(ground_truths) == len(datasets) == len(queries)
    logger.debug(f"[apply_verifiable_reward] is_same_length: {is_same_length}")
    
    # Log the datasets being processed
    unique_datasets = set(ds if isinstance(ds, str) else tuple(ds) for ds in datasets)
    logger.debug(f"[apply_verifiable_reward] unique datasets: {unique_datasets}")
    logger.debug(f"[apply_verifiable_reward] available verifiers: {list(reward_fn_mapping.keys())}")
    if verifier_weights:
        logger.debug(f"[apply_verifiable_reward] verifier_weights: {verifier_weights}")
        ...
        if isinstance(dataset, str):
            ...
            if len(ground_truth_list) != len(dataset_list):
                assert len(dataset_list) == 1, (
                    f"Ground truth and dataset list lengths do not match: "
                    f"{len(ground_truth_list)} vs {len(dataset_list)}"
                )
                ground_truth_list = [ground_truth]
    ...
    if async_tasks:
        try:
            reward_results = await asyncio.wait_for(
                asyncio.gather(*async_tasks),
                timeout=300.0
            )
        except asyncio.TimeoutError:
            logger.error("[apply_verifiable_reward] TIMEOUT")
            reward_results = [VerificationResult(score=0.0) for _ in async_tasks]
        except Exception as e:
            logger.error(f"[apply_verifiable_reward] Error gathering results: {e}")
            reward_results = [VerificationResult(score=0.0) for _ in async_tasks]
    else:
        logger.debug("[apply_verifiable_reward] No async tasks to gather")
        reward_results = []

    logger.debug(f"[apply_verifiable_reward] Reward results: {reward_results[:3]}...")
    response_rewards = [0] * len(responses)
    response_per_func_rewards = [{} for _ in range(len(responses))]
    response_raw_scores = [{} for _ in range(len(responses))]
    ...
    for result, metadata in zip(reward_results, task_metadata):
        ...
        # Check if we should use per-constraint weighting
        use_per_constraint = (
            verifier_weights is not None 
            and hasattr(result, "constraint_scores") 
            and result.constraint_scores is not None
        )
        
        if use_per_constraint:
            for constraint_name, constraint_score in result.constraint_scores.items():
                constraint_weight = verifier_weights.get(constraint_name, 0.0)
                
                if constraint_weight > 0:
                    weighted_reward = reward_mult * constraint_score * constraint_weight
                    response_rewards[response_idx] += weighted_reward
                    
                    response_per_func_rewards[response_idx][constraint_name] = (
                        response_per_func_rewards[response_idx].get(constraint_name, 0) + weighted_reward
                    )
                
                response_raw_scores[response_idx][constraint_name] = constraint_score
            
            score = result.score if hasattr(result, "score") else 0.0
            response_raw_scores[response_idx][dataset] = score
        else:
            score = result.score if hasattr(result, "score") else result            
            if verifier_weights is not None and dataset in verifier_weights:
                reward_weight = verifier_weights[dataset]
            ...
            response_raw_scores[response_idx][dataset] = (
                response_raw_scores[response_idx].get(dataset, 0) + score
            )

            if hasattr(result, "constraint_scores") and result.constraint_scores is not None:
                for constraint_name, constraint_score in result.constraint_scores.items():
                    response_raw_scores[response_idx][constraint_name] = constraint_score

        if hasattr(result, "extra_logging_scores") and result.extra_logging_scores is not None:
            for log_key, log_score in result.extra_logging_scores.items():
                response_raw_scores[response_idx][log_key] = log_score

    return response_rewards, response_per_func_rewards, response_raw_scores
...
class RewardConfig:
    ...
    binary_and_verifiers: list[str] | None = None

    # Per-constraint weights (keys are constraint names, e.g. "punctuation:no_comma").
    verifier_weights: dict[str, float] | None = None

    def __post_init__(self):
        if self.binary_and_verifiers and self.verifier_weights:
            raise ValueError(
                "verifier_weights and binary_and_verifiers are mutually exclusive. "
                "Set only one."
            )
    ...
    def build(self) -> Callable:
        ...
        async def reward_fn(
            ...
            logger.debug(f"[reward_fn] inside reward_fn with {len(responses)} responses")
            logger.debug(f"[reward_fn] with verifier_functions={self.verifier_functions}")
            ...
            if self.apply_verifiable_reward:
                verifiable_rewards, per_func_rewards, raw_scores = await apply_verifiable_reward(
                ...
                skip_weighted_metrics = self.binary_and_verifiers is not None and len(self.binary_and_verifiers) > 0
                
                if not skip_weighted_metrics:
                    np_verifiable_rewards = np.array(verifiable_rewards)
                    metrics["objective/verifiable_reward"] = np_verifiable_rewards.mean()
                    # metrics["objective/verifiable_correct_rate"] = (np_verifiable_rewards > 0.0).mean()
                    metrics["reward/verifiable_samples"] = np_verifiable_rewards  # Per-sample verifiable rewards
                    ...
                    for key, value in per_func_lists.items():
                        ...
                        metrics[f"reward/{key}_weighted_samples"] = np_value

                raw_score_lists: dict[str, list] = defaultdict(list)
                for raw_dict in raw_scores:
                    for key, value in raw_dict.items():
                        raw_score_lists[key].append(value)
                for key, value in raw_score_lists.items():
                    np_value = np.array(value)
                    metrics[f"objective/{key}_raw_reward"] = np_value.mean()
                    metrics[f"reward/{key}_raw_samples"] = np_value

                # binary_and_verifiers: reward is 1 only if all specified verifiers pass else 0
                if self.binary_and_verifiers is not None and len(self.binary_and_verifiers) > 0:
                    binary_scores = []
                    for i in range(len(scores)):
                        all_pass = True
                        failing_verifiers = []
                        for verifier_name in self.binary_and_verifiers:
                            verifier_key = verifier_name.lower()
                            if verifier_key not in raw_scores[i]:
                                all_pass = False
                                failing_verifiers.append(verifier_key)
                                continue
                            if raw_scores[i][verifier_key] < 1.0:
                                all_pass = False
                                failing_verifiers.append(verifier_key)
                        scores[i] = 1.0 if all_pass else 0.0
                        binary_scores.append(scores[i])
                    
                    np_binary_scores = np.array(binary_scores)
                    metrics["objective/binary_and_reward"] = np_binary_scores.mean()
                    metrics["objective/binary_and_pass_rate"] = (np_binary_scores == 1.0).mean()
