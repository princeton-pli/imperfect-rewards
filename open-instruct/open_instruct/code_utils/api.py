...
```
uv run nohup uvicorn open_instruct.code_utils.api:app --host 0.0.0.0 --port 1234 --workers 8 &
```
...
or launch using singularity:
```
apptainer build open_instruct/code_utils/code_api.sif open_instruct/code_utils/api.def
apptainer run --writable-tmpfs code_api.sif --host 0.0.0.0 --port 1234
```

and then test with:
```
python open_instruct/code_utils/api.py
```
