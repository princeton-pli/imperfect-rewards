...
from open_instruct import data_types, utils, grpo_utils
...
def should_log(args) -> bool:
    return bool(getattr(args, 'with_tracking', False)) and is_main_process()

def setup_file_logging(args, filename: str = 'logs/driver.log') -> str | None:
    if not is_main_process():
        return None

    logs_dir = os.path.join(args.output_dir, os.path.dirname(filename))
    os.makedirs(logs_dir, exist_ok=True)
    log_path = os.path.abspath(os.path.join(args.output_dir, filename))

    target_logger = logging.getLogger() if getattr(logger, 'propagate', True) else logger
...
def dump_run_args(args, tc, model_config, streaming_config, vllm_config, tools_config) -> None:
    if not is_main_process():
        return

    os.makedirs(args.output_dir, exist_ok=True)
    cmd_path = os.path.join(args.output_dir, 'command.txt')
    cmdline = ' '.join(shlex.quote(x) for x in sys.argv)
    pathlib.Path(cmd_path).write_text(cmdline + "\n", encoding='utf-8')
    ...
    payload = {
        ...
        'tools_config': {
            **{k: v for k, v in dataclasses.asdict(tools_config).items() if k != '_parsed_tools'},
            '_parsed_tools': [getattr(t, 'name', str(t)) for t in getattr(tools_config, '_parsed_tools', [])],
        },
    }

    json_path = os.path.join(args.output_dir, 'run_config.json')
    pathlib.Path(json_path).write_text(json.dumps(payload, indent=2, default=str) + "\n", encoding='utf-8')

    logger.info(f'Wrote run config to: {json_path}')
...
class Args:
    ...
    tensorboard_project_name: str = None
    """The tensorboard's project name"""
    tensorboard_flush_freq: int = 5
    """How often (in training steps) to flush TensorBoard events to disk."""
    ...
    """Whether to save per-sample reward traces"""
    save_traces_freq: int = 1
    """How often (in training steps) to save traces. E.g., 10 = save at steps 10, 20, 30..."""
    save_traces_generations: bool = False
    """Whether to include prompts/responses in traces"""
    consolidate_traces_freq: int = 100
    """How often to consolidate individual trace files into single files (to avoid too many files)"""
    ...
    # Success probability evaluation mode
    success_proba_only: bool = False
    """If True, run success probability evaluation on all datasets and exit without training.
    Uses datasets from both dataset_mixer_list and dataset_mixer_eval_list."""
    num_prompts: int = 500
    """Number of prompts to take from each dataset for success probability evaluation.
    The first num_prompts rows are taken in their original dataset order (no shuffling),
    so datasets that share the same underlying rows will get exactly the same prompts."""
    num_generations_per_prompt: int = 10
    """Number of generations per prompt for success probability evaluation."""
...
class PolicyTrainerRayProcess(RayProcess):
    ...
    def setup_model_update_group(self, vllm_engines):
        ...
        if self.rank == 0:
            ...
            if self._args.single_gpu_mode and backend == "nccl":
                logger.warning(
                    "single_gpu_mode is enabled with NCCL backend. "
                    "Switching to gloo backend for weight sync since NCCL doesn't support "
                    "multiple ranks on the same GPU."
                )
                backend = "gloo"
            refs = [
    ...
    @staticmethod
    def _per_sample_mean(values, mask, dim = 1) -> torch.Tensor:
        masked_vals = values * mask
        denom = mask.sum(dim=dim).clamp(min=1e-8)
        return (masked_vals.sum(dim=dim) / denom).float()

    def _to_tensor_sliced(self, x, num_samples_trained) -> torch.Tensor:
        t = (x.detach() if isinstance(x, torch.Tensor) else torch.tensor(x)).to(self.device).float().flatten()
        return t[:num_samples_trained]
        ...
        with Timer("[Training Processes] Loss calculation", noop=self.rank != 0):
            ...
            for epoch_idx in range(self.args.num_epochs):
                ...
                for i in range(num_samples):
                    ...
                    local_logprobs_BT = local_logprobs_BT.detach()
                    new_logprobs_BT = new_logprobs_BT.detach()
                    ratio_BT = ratio_BT.detach()
                    pg_losses_BT = pg_losses_BT.detach()
                    pg_losses2_BT = pg_losses2_BT.detach()
                    pg_loss_max_BT = pg_loss_max_BT.detach()
                    kl_BT = kl_BT.detach()
                    if tis_imp_ratio_BT is not None:
                        tis_imp_ratio_BT = tis_imp_ratio_BT.detach()
                    try:
                        per_seq_mean_ratio_B = masked_mean(ratio_BT, response_mask_BT, axis=-1)
                        self.local_metrics["logprobs/mean_ratio"] = per_seq_mean_ratio_B.mean()
                        self.local_metrics["logprobs/std_ratio"] = per_seq_mean_ratio_B.std(correction=0)
                    except (AttributeError, TypeError):
                        pass
                    if is_accumulation_boundary:
                        try:
                            self.local_metrics["debug/grad_norm"] = float(self.model.get_global_grad_norm())
                        except (AttributeError, TypeError):
                            pass
                        self.model.step()
                        torch.cuda.empty_cache()
...
def _bucket_mean(mask, vals):
    return vals[mask].mean().item() if mask.any() else float("nan")
...
def compute_reward_hacking_metrics(
    ...
    """Bucket samples by (format_correct, answer_correct) and compute stats per bucket"""
    r_fmt_f, r_ans_f = r_fmt.float().flatten(), r_ans.float().flatten()
    n = r_fmt_f.numel()
    if n == 0:
        return {}

    fmt_ok = r_fmt_f == 1.0
    ans_ok = r_ans_f == reward_correct

    buckets = {
        "fmt1_ans1": fmt_ok & ans_ok,
        "fmt1_ans0": fmt_ok & ~ans_ok,
        "fmt0_ans1": ~fmt_ok & ans_ok,
        "fmt0_ans0": ~fmt_ok & ~ans_ok,
    }

    metrics = {
        "marginal/r_fmt_mean": r_fmt_f.mean().item(),
        "marginal/r_ans_mean": r_ans_f.mean().item(),
        "count_bucket/total": float(n),
    }
    for name, mask in buckets.items():
        metrics[f"joint/pr_{name}"] = mask.float().mean().item()
        metrics[f"count_bucket/{name}"] = float(mask.sum().item())

    if advantages is not None:
        adv = advantages.float().flatten()
        adv_pos = adv > 0
        adv_pos_mass = torch.relu(adv)
        total_pos_mass = adv_pos_mass.sum().item()

        for name, mask in buckets.items():
            metrics[f"adv_bucket/{name}"] = _bucket_mean(mask, adv)
            metrics[f"adv_pos_rate/{name}"] = (adv_pos & mask).sum().item() / mask.sum().item() if mask.any() else float("nan")
            metrics[f"adv_pos_mass_share/{name}"] = adv_pos_mass[mask].sum().item() / total_pos_mass if total_pos_mass > 0 else float("nan")
...
def validate_configs(
    ...
    if vllm_config.vllm_reasoning_parser is not None and not streaming_config.enable_thinking:
        raise ValueError(
            f"vllm_reasoning_parser='{vllm_config.vllm_reasoning_parser}' is set but "
            "enable_thinking=False — the reasoning parser is only meaningful when thinking "
            "tokens are present in the model output.  Either set enable_thinking=True or "
            "remove vllm_reasoning_parser."
        )
    if streaming_config.enable_thinking and vllm_config.vllm_reasoning_parser is None:
        logger.warning(
            "enable_thinking=True but vllm_reasoning_parser is not set.  The vLLM server "
            "will not parse reasoning content from the model output.  Consider setting "
            "vllm_reasoning_parser (e.g. 'qwen3') if you need structured reasoning output."
        )
...
def setup_runtime_variables(
    ...
    if args.with_tracking and is_main_process():
        if args.wandb_entity is None and args.tensorboard_project_name is None:
            args.wandb_entity = maybe_use_ai2_wandb_entity()
        elif args.tensorboard_project_name is not None:
            args.wandb_entity = None
            args.wandb_project_name = None
    return args
...
def setup_experiment_tracking(args: Args, tc: TokenizerConfig, model_config: ModelConfig):
    ...
    if args.with_tracking and is_main_process():
        if args.wandb_project_name is not None:
            ...
            global TB_WRITER, TB_LOG_DIR
            TB_LOG_DIR = os.path.join(
                args.output_dir, 'tensorboard', str(args.tensorboard_project_name)
            )
            os.makedirs(TB_LOG_DIR, exist_ok=True)
            TB_WRITER = SummaryWriter(log_dir=TB_LOG_DIR)
            TB_WRITER.add_text('run/name', str(args.run_name), 0)
            TB_WRITER.add_text('config/all', json.dumps(all_configs, indent=2, default=str), 0)
            TB_WRITER.flush()
            logger.info(f'TensorBoard logging enabled. log_dir={TB_LOG_DIR}')
...
def setup_datasets(
    ...
    transform_fn_args = [
        {
            ...
            "enable_thinking": streaming_config.enable_thinking,
        },
    ...
    train_dataset = get_cached_dataset_tulu(
        ...
        dataset_config_seed=streaming_config.dataset_config_seed,
        system_prompt_override=system_prompt_override,
    )

    # We need to make sure to preserve dataset[i]["index"] == i !!
    ...
    if len(streaming_config.dataset_mixer_eval_list) > 0:
        ...
        if streaming_config.shuffle_eval_dataset:
            logger.warning("Shuffling evaluation dataset is disabled.")
...
def create_model_and_optimizer(
    ...
    bundles = [{"GPU": actor_num_gpus, "CPU": actor_num_gpus * 5} for actor_num_gpus in args.num_learners_per_node] #put 10 for training;  debug with 5 on head node
    ...
    # Use the wandb_url provided by setup_experiment_tracking (may be None when W&B is disabled)
    ...
    vllm_engines = vllm_utils.create_vllm_engines(
        ...
        reasoning_parser=vllm_config.vllm_reasoning_parser,
    )
...
def one_training_step(
    ...
    sum_metrics_prefixes = (
        "reward_hacking/count_bucket/",
    )
    for k in metrics[0]:
        if k == "_token_count":
            # Don't include internal token count in final metrics
            continue
        if any(k.startswith(p) for p in sum_metrics_prefixes):
            # Counts should be summed across ranks/workers
            average_metrics[k] = sum(m[k] for m in metrics)
            continue
    ...
    if should_log(args):
        loggable_metrics = {k: v for k, v in metrics.items() if not k.startswith("trace/")}
        loggable_scalar_metrics = {k: v for k, v in scalar_metrics.items() if not k.startswith("trace/")}
        
        if args.wandb_project_name is not None and getattr(wandb, 'run', None) is not None:
            wandb_metrics = dict(loggable_metrics)
        ...
        if TB_WRITER is not None:
            tb_step = training_step
            for k, v in loggable_scalar_metrics.items():
                TB_WRITER.add_scalar(k, float(v), tb_step)
            for key, value in loggable_metrics.items():
                if isinstance(value, (np.ndarray, list)) and len(value) > 0:
                    TB_WRITER.add_histogram(key, np.asarray(value), tb_step)
            if args.tensorboard_flush_freq and args.tensorboard_flush_freq > 0 and (
                tb_step % args.tensorboard_flush_freq == 0
            ):
                TB_WRITER.flush()

    try:
        maybe_save_traces(args, training_step, "train", metrics={**metrics, **array_metrics[0]})
        if args.save_traces and training_step % args.consolidate_traces_freq == 0:
            consolidate_traces(args, final=False)
    except Exception as e:
        logger.warning(f"Failed to save/consolidate train traces at step {training_step}: {e}")
...
def maybe_save_traces(
    args: Args,
    training_step: int,
    trace_type: Literal["train", "eval"],
    metrics: dict[str, Any],
    prompts: list[str] | None = None,
    responses: list[str] | None = None,
    ground_truths: list[Any] | None = None,
) -> None:
    """Save per-sample reward traces to: traces/{train|eval}/step_{step:06d}.pt
    
    Saves:
    - reward/*_samples: per-sample rewards (raw and weighted) for each verifier
    - trace/scores: final training scores
    """
    if not args.save_traces:
        return
    if trace_type == "train" and training_step % args.save_traces_freq != 0:
        return
    if not is_main_process():
        return
    
    traces_dir = os.path.join(args.output_dir, "traces", trace_type)
    os.makedirs(traces_dir, exist_ok=True)
    step_data = {"step": training_step}
    
    for key, value in metrics.items():
        if key.startswith("reward/") and "_samples" in key:
            step_data[key] = to_tensor(value)
        elif key == "trace/scores":
            step_data["scores"] = to_tensor(value)
    
    if args.save_traces_generations:
        if prompts is not None:
            step_data["prompts"] = prompts
        if responses is not None:
            step_data["responses"] = responses
        if ground_truths is not None:
            step_data["ground_truths"] = ground_truths
    
    trace_path = os.path.join(traces_dir, f"step_{training_step:06d}.pt")
    torch.save(step_data, trace_path)
    logger.debug(f"Saved {trace_type} traces to {trace_path}")


def consolidate_traces(args: Args, final: bool = False) -> None:
    """Consolidate all individual trace files into single files at end of training."""
    if not args.save_traces:
        return
    if not is_main_process():
        return
    
    traces_base = os.path.join(args.output_dir, "traces")
    
    for subdir, output_name in [("train", "train_traces.pt"), ("eval", "eval_traces.pt")]:
        subdir_path = os.path.join(traces_base, subdir)
        trace_files = sorted(glob.glob(os.path.join(subdir_path, "step_*.pt")))
        logger.info(f"Consolidating {len(trace_files)} {subdir} trace files...")
        output_path = os.path.join(traces_base, output_name)
        if os.path.exists(output_path):
            consolidated = torch.load(output_path, weights_only=False)
        else:
            consolidated = {}
        
        for trace_file in trace_files:
            try:
                data = torch.load(trace_file, weights_only=False)
                step = data.get("step", int(os.path.basename(trace_file).split("_")[1].split(".")[0]))
        ...
        if consolidated:
            ...
            logger.info(f"Saved consolidated traces to {output_path} ({len(consolidated)} steps)")
...
def maybe_evaluate(
    ...
    model_dims: utils.ModelDims,
    actor_manager=None,
    expect_eval_results: bool = False,
):
    def tb_inline(x, max_len=600):
        if x is None:
            return ""
        s = html.escape(str(x))
    ...
    try:
        if training_step >= args.num_training_steps:
            timeout = 600
        elif expect_eval_results:
            timeout = 600
        else:
            timeout = 0.01
        
        logger.debug("[Main Thread] ⏳ Waiting for evaluation responses. Timeout is %s seconds", timeout)
        ...
        eval_reward_metrics_for_traces = dict(eval_reward_metrics)
        eval_reward_metrics_for_traces["trace/scores"] = eval_batch.scores
        ...
        table = {}
        table["prompt"] = tokenizer.batch_decode(eval_batch.queries if eval_batch else [], clean_up_tokenization_spaces=False)
        table["response"] = eval_batch.decoded_responses
        table["response"] = [item.replace(tokenizer.pad_token, "") for item in table["response"]]
        table["scores"] = eval_batch.scores
        table["ground_truth"] = eval_batch.ground_truths if eval_batch else []

        strict_boxed_key = "reward/strict_boxed_math_raw_samples" # Check frr parsing
        if strict_boxed_key in eval_reward_metrics_for_traces:
            raw = eval_reward_metrics_for_traces[strict_boxed_key]
            table["strict_boxed_math"] = raw.tolist() if isinstance(raw, np.ndarray) else list(raw)
        df = pd.DataFrame(table)

        print_rich_table(df.iloc[:min(15, len(df))])
        ...
        try:
            maybe_save_traces(
                ...
                training_step,
                "eval",
                metrics=eval_reward_metrics_for_traces,
                prompts=table["prompt"],
                responses=table["response"],
                ground_truths=table["ground_truth"],
            )
        except Exception as e:
            logger.warning(f"Failed to save eval traces at step {training_step}: {e}")

        if should_log(args):
            if args.wandb_project_name is not None and getattr(wandb, 'run', None) is not None:
                eval_wandb = dict(eval_metrics)
                eval_wandb['sample_completions'] = wandb.Table(dataframe=df)
                wandb.log(eval_wandb, step=episode)
            ...
            if TB_WRITER is not None:
                ...
                if len(df) > 0:
                    ...
                    for i in range(num_samples):
                        ...
                        blocks.append(
                            f"**Sample {i}**\n"
                            f"- **Score:** {row.get('scores', 'N/A')}\n"
                            f"- **Prompt:**\n<pre>{tb_inline(row.get('prompt',''))}</pre>\n"
                            f"- **Response:**\n<pre>{tb_inline(row.get('response',''))}</pre>\n"
                            f"- **Ground truth:**\n<pre>{tb_inline(row.get('ground_truth',''))}</pre>\n"
                        )
                    TB_WRITER.add_text("eval/samples", "\n\n---\n\n".join(blocks), tb_step)
                if args.tensorboard_flush_freq and args.tensorboard_flush_freq > 0 and (
                    tb_step % args.tensorboard_flush_freq == 0
                ):
                    TB_WRITER.flush()
        ...
        logger.error("[Main Thread] 💥 Error in maybe_evaluate: %s", e)
...
def run_training(
    ...
    if args.eval_on_step_0 and resume_training_step == 1 and eval_dataset is not None:
        logger.info("[Main Thread] 📊 Starting initial policy evaluation (step 0)")
        ...
        maybe_evaluate(
            ...
            model_dims,
            actor_manager,
            expect_eval_results=True,
        )
        logger.info("[Main Thread] ✅ Initial policy evaluation complete")
    ...
    for training_step in range(resume_training_step, args.num_training_steps + 1):
        ...
        submitted_eval_prompts = (
            training_step % args.local_eval_every == 0
            and eval_data_loader is not None
            and (args.eval_on_step_0 or training_step > 1)
        )
        if submitted_eval_prompts:
            for eval_example in eval_dataset:
                add_prompt_to_generator(eval_example, 0, prompt_Q, generation_configs["eval"], is_eval=True)
        ...
        maybe_evaluate(
            ...
            model_dims,
            actor_manager,
            expect_eval_results=submitted_eval_prompts,
        )
    ...
    try:
        consolidate_traces(args, final=True)
    except Exception as e:
        logger.warning(f"Failed to consolidate traces: {e}")


def run_success_proba_evaluation(
    args: Args,
    tc: TokenizerConfig,
    tokenizer: PreTrainedTokenizer, 
    model_config: ModelConfig,
    ...
    """Run success probability evaluation on all datasets and exit"""

    logger.info("=" * 60)
    logger.info("Running success probability evaluation (success_proba_only=True)")
    logger.info(f"num_prompts={args.num_prompts}, num_generations_per_prompt={args.num_generations_per_prompt}")
    logger.info("=" * 60)

    verifier_weights_dict = json.loads(streaming_config.verifier_weights)
    constraint_names = [key for key in verifier_weights_dict.keys()]

    ####### Parse individual datasets from mixer lists as done in training flow ######
    dataset_configs = []
    train_mixer = streaming_config.dataset_mixer_list
    train_splits = streaming_config.dataset_mixer_list_splits
    if len(train_splits) == 1:
        train_splits = train_splits * len(train_mixer)
    for i in range(0, len(train_mixer), 2):
        dataset_configs.append((train_mixer[i], train_splits[i]))

    eval_mixer = streaming_config.dataset_mixer_eval_list
    eval_splits = streaming_config.dataset_mixer_eval_list_splits
    if len(eval_splits) == 1:
        eval_splits = eval_splits * len(eval_mixer)
    for i in range(0, len(eval_mixer), 2):
        dataset_configs.append((eval_mixer[i], eval_splits[i]))

    seen = set()
    unique_configs = []
    for name, split in dataset_configs:
        key = (name, split)
        if key not in seen:
            seen.add(key)
            unique_configs.append((name, split))

    for name, split in unique_configs:
        logger.info(f"- {name} (split={split})")
    ...
    for name, split in unique_configs:
        ds_key = f"{name}::{split}"
        logger.info(f"Loading dataset {ds_key}...")

        ds = get_cached_dataset_tulu(
            dataset_mixer_list=[name, "1.0"],
        ...
        # no shuffling; it is done when we create the dataset through IFBench. 
        n_available = len(ds)
        n_select = min(args.num_prompts, n_available)
        ds = ds.select(range(n_select))

        logger.info(f"Selected first {n_select}/{n_available} prompts from {ds_key} (original order preserved)")
        per_dataset_data[ds_key] = ds
        all_selected.append(ds)

    combined_dataset = concatenate_datasets(all_selected)
    combined_dataset = combined_dataset.map(
        lambda example, idx: {**example, "index": idx},
        with_indices=True,
    )
    logger.info(f"Combined evaluation dataset: {len(combined_dataset)} prompts total")

    ####### Create minimal infra ######
    total_prompts = len(combined_dataset)
    queue_size = total_prompts + 100
    ...
    success_proba_gen_config = generation_configs["train"]
    success_proba_gen_config.n = args.num_generations_per_prompt
    # vllm_utils.SamplingConfig(
    #     temperature=streaming_config.temperature,
    #     top_p=1.0,
    #     max_tokens=streaming_config.response_length,
    ...
    logger.info("Creating vLLM engines for success probability evaluation...")
    vllm_engines = vllm_utils.create_vllm_engines(
        vllm_config.vllm_num_engines,
        vllm_config.vllm_tensor_parallel_size,
        vllm_config.vllm_enforce_eager,
    ...
    logger.info("vLLM engines initialized for success probability evaluation")
    ray.get(actor_manager.set_should_stop.remote(False))

    ###### Submit all prompts for generation ######
    logger.info(
        f"Submitting {total_prompts} prompts for generation "
        f"(n={args.num_generations_per_prompt} generations each)..."
    )
    for example in combined_dataset:
        add_prompt_to_generator(example, 0, prompt_Q, success_proba_gen_config, is_eval=False)
    ...
    while num_collected < total_prompts:
        try:
            result = inference_results_Q.get(timeout=600)
        except Exception as e:
            raise Exception(
                f"Timeout or error waiting for results "
                f"(collected={num_collected}/{total_prompts}): {e}"
            )

        if isinstance(result, data_types.ShutdownSentinel):
            raise Exception("Received shutdown sentinel during success proba evaluation!")
        results_by_index[result.index] = result
        num_collected += 1
        if num_collected % 50 == 0 or num_collected == total_prompts:
            logger.info(f"Collected {num_collected}/{total_prompts} results")

    logger.info(f"Collected {num_collected}/{total_prompts} results")

    ###### Process results per dataset ######
    output_dir = os.path.join(args.output_dir, "success_proba")
    os.makedirs(output_dir, exist_ok=True)

    all_stats = {}
    offset = 0

    for ds_key, ds in per_dataset_data.items():
        n_prompts = len(ds)
        logger.info(f"{'='*40}")
        logger.info(f"Processing dataset: {ds_key} ({n_prompts} prompts)")
        logger.info(f"{'='*40}")
        ds_stats = {}
        raw_tensors = {}
        n_gens = args.num_generations_per_prompt
        table = {"prompt": [], "response": []}
        ds_results = []

        for i in range(offset, offset + n_prompts):
            ds_results.append(results_by_index[i])
            table["prompt"].append(
                tokenizer.decode(combined_dataset[i][INPUT_IDS_PROMPT_KEY], clean_up_tokenization_spaces=False)
            )
            decoded_gen = tokenizer.batch_decode(results_by_index[i].responses, clean_up_tokenization_spaces=False)
            table["response"].append([item.replace(tokenizer.pad_token, "") for item in decoded_gen][-1]) #taking a random element to display

        if args.save_traces_generations:
            raw_tensors["prompt_generations"] = table
                
        df = pd.DataFrame(table)
        print_rich_table(df.iloc[:min(5, len(df))])
        logger.info(df.iloc[:min(5, len(df))].to_string())

        for constraint_name in constraint_names:
            key = f"reward/{constraint_name}_raw_samples"
            rows = []
            for result in ds_results:
                scores = result.reward_metrics[key]
                rows.append(torch.tensor(scores, dtype=torch.float32))

            raw_matrix = torch.stack(rows)  # (num_prompts_actual, num_generations)

            # Per-prompt success rate
            assert ((raw_matrix == 0.0) | (raw_matrix == 1.0)).all(), "raw_samples must contain only 0 or 1"
            per_prompt_success_rate = raw_matrix.mean(dim=1)

            mean_rate = float(per_prompt_success_rate.mean())
            std_rate = float(per_prompt_success_rate.std())
            q1_rate = float(torch.quantile(per_prompt_success_rate, 0.25))
            q3_rate = float(torch.quantile(per_prompt_success_rate, 0.75))

            ds_stats[constraint_name] = {
                "mean": mean_rate,
                "std": std_rate,
                "q1": q1_rate,
                "q3": q3_rate,
                "num_prompts": len(ds_results),
                "num_generations_per_prompt": n_gens,
            }
            raw_tensors[constraint_name] = raw_matrix

            logger.info(
                f"Constraint '{constraint_name}': "
                f"mean={mean_rate:.4f}, std={std_rate:.4f}, "
                f"q1={q1_rate:.4f}, q3={q3_rate:.4f}"
            )

        # Combined success rate (all constraints satisfied)
        all_success = torch.ones(len(ds_results), n_gens)
        for constraint_name in constraint_names:
            all_success *= raw_tensors[constraint_name]

        per_prompt_combined = all_success.mean(dim=1)
        combined_stats = {
            "mean": float(per_prompt_combined.mean()),
            "std": float(per_prompt_combined.std()),
            "q1": float(torch.quantile(per_prompt_combined, 0.25)),
            "q3": float(torch.quantile(per_prompt_combined, 0.75)),
            "num_prompts": len(ds_results),
            "num_generations_per_prompt": n_gens,
        }
        ds_stats["__combined__"] = combined_stats
        raw_tensors["__combined__"] = all_success

        logger.info(
            f"Combined (all constraints): "
            f"mean={combined_stats['mean']:.4f}, std={combined_stats['std']:.4f}, "
            f"q1={combined_stats['q1']:.4f}, q3={combined_stats['q3']:.4f}"
        )
        all_stats[ds_key] = ds_stats

        ds_safe_name = ds_key.replace("/", "_").replace("::", "__")
        ds_output_dir = os.path.join(output_dir, ds_safe_name)
        os.makedirs(ds_output_dir, exist_ok=True)

        for constraint_name, tensor in raw_tensors.items():
            safe_name = constraint_name.replace("/", "_").replace(":", "_")
            if isinstance(tensor, dict):
                tensor_path = os.path.join(ds_output_dir, f"prompt_generations.pt")
                torch.save(tensor, tensor_path)
                logger.info(f"Saved prompt generations -> {tensor_path}")
            else:
                tensor_path = os.path.join(ds_output_dir, f"raw_rewards_{safe_name}.pt")
                torch.save(tensor, tensor_path)
                logger.info(f"Saved raw rewards for '{constraint_name}': shape {tuple(tensor.shape)} -> {tensor_path}")

        offset += n_prompts

    summary_path = os.path.join(output_dir, "success_proba_summary.json")
    ...
    for q in [prompt_Q, inference_results_Q, evaluation_inference_results_Q]:
        try:
            q.shutdown()
        except Exception:
            pass
    ...
    logger.info("=" * 60)
    logger.info("Success probability evaluation complete!")
    logger.info("=" * 60)
...
def main(
    ...
    if not args.success_proba_only:
    ...
    dump_run_args(args, tc, model_config, streaming_config, vllm_config, tools_config)
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as _s:
        _s.bind(("", 0))
        ray_gcs_port = _s.getsockname()[1]
    os.environ["RAY_PORT"] = str(ray_gcs_port)
    ...
    ray_init_kwargs = dict(
        ...
        num_cpus=int(os.environ.get("SLURM_CPUS_PER_TASK", "10")), #default is 10
    )
    ...
    verifier_weights_dict = None
    if streaming_config.verifier_weights is not None:
        try:
            verifier_weights_dict = json.loads(streaming_config.verifier_weights)
            logger.info(f"Parsed verifier_weights: {verifier_weights_dict}")
            total_weight = sum(verifier_weights_dict.values())
            if abs(total_weight - 1.0) != 0.:
                logger.warning(f"verifier_weights sum to {total_weight}, not 1.0. Consider normalizing.")
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON in verifier_weights: {e}")
    ...
    reward_config = RewardConfig(
        ...
        binary_and_verifiers=streaming_config.binary_and_verifiers,
        verifier_weights=verifier_weights_dict,
    )
    ...
    if args.success_proba_only:
        try:
            run_success_proba_evaluation(
                args=args,
                tc=tc,
                tokenizer=tokenizer,
                model_config=model_config,
            ...
            if is_main_process() and TB_WRITER is not None:
                TB_WRITER.flush()
                TB_WRITER.close()
        logger.info("Success proba evaluation done. Exiting without training.")
        return
