...
class VLLMConfig:
    ...
    vllm_reasoning_parser: str | None = None #--reasoning-parser flag of vllm serve
...
class StreamingDataLoaderConfig:
    ...
    binary_training: bool = False  # if True, only return 0 or 1 rewards based on pass_rate_reward

    # Max length verifier
    max_length_verifier_max_length: int = 32768

    # GSM8K strict verifier answer format: "gsm8k" (####) "boxed" (\boxed{}) or "auto"
    gsm8k_answer_format: str = "auto"

    # Non stop penalty
    non_stop_penalty: bool = False
    non_stop_penalty_value: float = 0.0

    # if set, reward is 1 only if ALL listed verifiers have raw score == 1, else 0
    binary_and_verifiers: list[str] | None = None

    # Per verifier weights
    # Example: '{"gsm8k_strict": 0.5, "punctuation:no_comma": 0.25, "length:at_least_n_words": 0.25}'
    verifier_weights: str | None = None
...
def accumulate_inference_batches(
    ...
    while num_prompts_sampled < num_prompts:
        ...
        decoded_responses = tokenizer.batch_decode(result.responses, skip_special_tokens=True, clean_up_tokenization_spaces=False)
...
class DataPreparationActor:
    ...
    def __init__(
        ...
        self.training_step = 0
        self.consumed_step = -1

        if initial_state is not None:
            self.training_step = initial_state["training_step"]
            self.consumed_step = initial_state["training_step"] - 1
    ...
    def _data_preparation_loop(self):
        ...
        for step in range(self.training_step, self.num_training_steps):
            ...
            # don't run more than async_steps ahead of what training has consumed
            max_ahead = self.config.async_steps
            wait_count = 0
            while not self.shutdown_requested:
                with self.lock:
                    if step <= self.consumed_step + max_ahead:
                        break
                wait_count += 1
                if wait_count % 100 == 0:
                    logger.info(
                        f"[DataPreparationActor] Step {step}: waiting for training to catch up "
                        f"(consumed_step={self.consumed_step}, max_ahead={max_ahead})"
                    )
                time.sleep(0.1)
            if self.shutdown_requested:
                return
            ...
            if len(result.responses) == 0:
                ...
                step_metrics["trace/prompts"] = batch.raw_queries
                step_metrics["trace/responses"] = batch.decoded_responses
                step_metrics["trace/ground_truths"] = batch.ground_truths
                step_metrics["trace/scores"] = scores.tolist()
        ...
        while True:
            ...
            with self.lock:
                if step <= self.current_prepared_step:
                    ...
                    if step > self.consumed_step:
                        self.consumed_step = step
                    logger.info(
