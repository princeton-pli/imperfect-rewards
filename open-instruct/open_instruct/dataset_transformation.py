...
# monkey patch for my offline mode problem
if os.environ.get("HF_HUB_OFFLINE", "0") == "1":
    try:
        print("Monkey patch huggingface_hub.model_info to handle offline mode")
        ...
        huggingface_hub.hf_api.model_info = _patched_model_info
    except Exception as e:
        print(f"Warning: Failed to patch model_info: {e}")
...
def get_tokenizer_tulu_v1(tc: "TokenizerConfig"):
    ...
    if isinstance(tokenizer, LlamaTokenizer) or isinstance(tokenizer, LlamaTokenizerFast):
        ...
        if tokenizer.pad_token is None or tokenizer.pad_token_id == tokenizer.eos_token_id:
            if "<|finetune_right_pad_id|>" in tokenizer.get_vocab():
                ...
                raise ValueError("For LlamaTokenizer, we are using <|finetune_right_pad_id|>")
        assert num_added_tokens in [0], (
            "LlamaTokenizer should not add any tokens - we are using existing token for padding."
        )
    ...
    #     num_added_tokens = tokenizer.add_special_tokens({"unk_token": "<unk>"})
        ...
        if "llama" in tc.tokenizer_name_or_path.lower():
            print("Detected LLaMA tokenizer with no pad token. Trying to add <|finetune_right_pad_id|> as the pad token.")
            if "<|finetune_right_pad_id|>" in tokenizer.get_vocab():
                tokenizer.pad_token = "<|finetune_right_pad_id|>"
            else:
                raise ValueError("For LlamaTokenizer, we are using <|finetune_right_pad_id|>")
    ...
    if tokenizer.pad_token_id is None or tokenizer.pad_token_id == tokenizer.eos_token_id:
        ...
        #     num_added_tokens = tokenizer.add_special_tokens({"unk_token": "<unk>"})
            ...
            if "llama" in tc.tokenizer_name_or_path.lower():
                ...
                num_added_tokens = tokenizer.add_special_tokens({"pad_token": "<pad>"})
                print("[WARNING] Adding new pad token")
                assert num_added_tokens == 1, "We detected no padding token but add_special_tokens did not add one."
...
def rlvr_tokenize_v1(
    ...
    row[INPUT_IDS_PROMPT_KEY] = tokenizer.apply_chat_template(
        prompt, add_generation_prompt=True, enable_thinking=enable_thinking
    )
    row[INPUT_IDS_KEY] = tokenizer.apply_chat_template(row[sft_messages_key], enable_thinking=enable_thinking)
    row[ATTENTION_MASK_KEY] = [1] * len(row[INPUT_IDS_KEY])
    labels = copy.deepcopy(row[INPUT_IDS_KEY])
    row[LABELS_KEY] = labels
...
def rlvr_tokenize_v2(
    ...
    if isinstance(row[verifier_source_key], str):
        row[ground_truths_key] = [row[ground_truths_key]]
        row[verifier_source_key] = [row[verifier_source_key]]
    elif isinstance(row[ground_truths_key], str):
        row[ground_truths_key] = [row[ground_truths_key]]
...
def get_dataset_v1(dc: DatasetConfig, tc: TokenizerConfig):
    ...
    for fn_name, fn_args in zip(dc.transform_fn, dc.transform_fn_args):
        ...
        # It's pretty perturbing because some tokenizer have clean_up_tokenization_spaces as True. So it can clean up spaces in decoding for pretty print but nothing is actually wrong.
