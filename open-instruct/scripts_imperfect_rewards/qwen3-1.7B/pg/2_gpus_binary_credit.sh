...
OUT_DIR="pg_vr/qwen_1p7B/${PAIR_SLUG}/${RUN_DATE}/binary_credit/"
mkdir -p "$OUT_DIR"
...
python open_instruct/grpo_fast.py \
  ...
  --num_samples_per_prompt_rollout 96 \
  --temperature 1 \
  --num_mini_batches 1 \
  --kl_estimator 2 \
  ...
  --binary_and_verifiers ifeval \
  --verification_reward 1 \
  --non_stop_penalty False \
  --total_episodes 3072000 \
  --deepspeed_stage 2 \
  ...
  --with_tracking True \
  --tensorboard_project_name "${EXP_NAME}" \
  --push_to_hub False \
