...
class ScalarMetric(Metric, ABC):
    """
    Scalar metric marker class.
    """
    pass


class AveragedMetric(ScalarMetric, ABC):
    """
...
class MinMetric(ScalarMetric, ABC):
    """
    Metric abstract parent class for metrics that are obtained by taking the minimum over all of the samples.
    """
    ...
    def __call__(self, *args, **kwargs):
        metric_value = self._calc_metric(*args, **kwargs)
        self.current_min = min(self.current_min, metric_value)
        return metric_value

    @abstractmethod
    def _calc_metric(self, *args, **kwargs):
        """
        Calculates the metric value for the given input and returns its value.
        """
        raise NotImplementedError
    ...
    def has_epoch_metric_to_update(self):
        return self.current_min != math.inf

    def reset_current_epoch_values(self):
        self.current_min = math.inf


class MaxMetric(ScalarMetric, ABC):
    """
    Metric abstract parent class for metrics that are obtained by taking the maximum over all of the samples.
    """
    ...
    def __call__(self, *args, **kwargs):
        metric_value = self._calc_metric(*args, **kwargs)
        self.current_max = max(self.current_max, metric_value)
        return metric_value
    ...
    def has_epoch_metric_to_update(self):
        return self.current_max != -math.inf
