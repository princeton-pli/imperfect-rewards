...
from imperfect_rewards.arguments.args import CustomRLOOConfig
from imperfect_rewards.data.utils.rm_utils import normalize_rewards
from imperfect_rewards.utils import is_main_process, get_logger, batch_generation_no_model_defaults
...
class CustomRLOOTrainer(RLOOTrainer):
    ...
    def __get_reward_with_correct_tokenizer(self, batch_prompts, query_tokens, response_tokens, pad_token_id,
        ...
        if self.reward_model_tokenizer is None:
            ...
            chat_messages = self.reward_model_tokenizer.apply_chat_template(chat_messages, tokenize=False, enable_thinking=False)
        ...
        score = normalize_rewards(score, self.reward_shift, self.reward_scale)
        return score
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            with torch.no_grad():
                ...
                with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                    query_responses, logitss = batch_generation_no_model_defaults(
                        unwrapped_model,
                        queries,
            ...
            if args.early_stopping and args.target_kl:
                # Update KL history for smoothing
                self.kl_history.append(metrics["objective/kl"])
                if len(self.kl_history) > args.kl_smoothing_interval_for_early_stopping:
                    self.kl_history.pop(0)

                # Use smoothed KL estimate for early stopping decision
                if len(self.kl_history) >= args.kl_smoothing_interval_for_early_stopping:
                    smoothed_kl = sum(self.kl_history) / len(self.kl_history)
                    if smoothed_kl > args.target_kl + 1e-6:
                        accelerator.print(
                            f"===early stopping due to KL stopping criteria (smoothed KL: {smoothed_kl:.6f} > target: {args.target_kl})===")
                        break
    ...
    def generate_completions_using_device(self, device):
        ...
        with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
            ...
            with torch.no_grad():
                ...
                query_response, _ = batch_generation_no_model_defaults(
                    unwrapped_model,
                    query,
                    query.shape[0],
                    processing_class.pad_token_id,
